...
import { UserRound, Mail, MessageSquare, FolderKanban, Users, Sparkles, ArrowRight } from 'lucide-vue-next'
...
const isEmailValid = computed(() => formData.value.email.includes('@') && formData.value.email.includes('.'))
const isMessageValid = computed(() => formData.value.message.trim().length >= 10)
const isFormValid = computed(() => isFullNameValid.value && isEmailValid.value && isMessageValid.value)
</script>
...
<template>
  <section id="contact-section" class="relative bg-slate-950 py-24">
    ...
    <div class="relative z-10 max-w-6xl mx-auto px-6 flex flex-col items-center text-center">
      ...
      <form autocomplete="off" @submit.prevent="handleSubmit" class="w-full max-w-md p-8 rounded-2xl border border-slate-800 bg-slate-900 mt-8">
